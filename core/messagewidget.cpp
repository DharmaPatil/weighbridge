...
bool MessageModel::showMessage(Workbench *wb, int channel, int type, const QString &text, bool show)
{
	...
	if (show)
	{
		...
		msg.restored = false;
		msg.time = now;
		msg.fullText = QString("%1 %2 %3")
				.arg(msg.time.toString())
				.arg(wb->instrInfo("title").toString())
				.arg(msg.text);
		m_msgIndex.insert(msg.text, rowCount(QModelIndex()));
		m_messages.append(msg);

        if (type < m_count.size())
			++m_count[type];
	...
	else
	{
		...
		foreach(int i, indices)
		{
			...
			// 报警恢复了
            if (type < m_count.size() && m_count[type] > 0)
				--m_count[type];
    ...
    ui(new Ui::MessageWidget)
{
	...
	m_toggleAction = new QAction(tr("显示/隐藏信息"), this);
